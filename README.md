...
画面（URL）は**1つ**です。ページ遷移はせず、投稿フォームの下に「提供します」「求めています」の2タブが並びます。「🔍 ラベルを探す」はこの2つとは別に、右下の丸いアイコンから開きます。
...
ラベルのボタンを押すと表示が切り替わります。すべてオフにすれば学内の全投稿が見えます。右端の「すべて選択／すべて解除」は、今見えているラベルを一括でON・OFFする切り替えボタンです（押すたびに反転します）。
...
- カテゴリ（学部・学年・履修科目・課外活動・関心・キャンパス）ごとに見出しが付き、ラベルが増えても迷わず探せます
...
タブに揃えた直後は「提供します」「求めています」「🔍 ラベルを探す」の3つを同じタブ列に並べていました。動かしてみると、この3つは実は同格ではないことに気づきました。**「提供します」「求めています」は中身を見るタブ（コンテンツビュー）ですが、「ラベルを探す」は中身を見るための条件を作る道具（フィルターの設定）**です。道具と中身が同じタブ列に並ぶと、3つの同格な画面があるように見えてしまいます。

そこで「ラベルを探す」だけタブから外し、**右下に常時表示する丸いアイコンから、オーバーレイ（画面にかぶさるパネル）で開く**形に変えました。中身（学部・学年のプルダウン、シラバスなど）は変えていません。開き方と位置だけを変えています。

以前、検索欄が見にくく気づかれにくいという問題があってタブ化しましたが、その原因は「タブでなかったこと」自体ではなく、「地味なボタン1つで目立たなかったこと」でした。右下に固定で目立つアイコンを置くことで、タブにしなくても見つけやすさは保てます。

**検索の中には「タイムライン」がありません。** ラベルを選んだだけでは、その効果がオーバーレイの中では見えないままです。そこで、選ぶたびに**今の絞り込みで何件見つかるか**をオーバーレイの上部にそのまま表示し、押すと「提供します」「求めています」のタブへ直接移動（同時にオーバーレイも閉じる）できるようにしています。
...
公開か非公開かとは別に、**投稿者・返信者は自分の判断で名前を隠せます。** チェックを入れるだけで、自分以外には「匿名」と表示されます。

- 名乗りたくないが聞きたい・答えたい話（教職課程がどれくらい大変か、成績のことなど）に使えることを想定しています
- 投稿する本人には、匿名にしていても自分の投稿だと分かります（自分にだけは実名のまま表示）
- 内部的には、匿名にしても投稿者・返信者のidは普段どおり記録されています。**匿名は表示だけの機能**で、非公開の返信の可視性判定や、状態変更ができるかどうかの判定には一切影響しません

### 自分の投稿に返信が来たら、ベルで知らせます

サーバーがないので、通知を本当にプッシュすることはできません。その代わり、**画面右上の🔔に、自分の投稿への返信が未読件数として出ます。** 開くと一覧が見え、開いた時点で既読になります。

- トリガーは「自分の投稿に、自分以外から返信が来たこと」だけです。公開・非公開どちらの返信でも対象になります（投稿者は元々どちらも見られるため、§7冒頭）
- 匿名の返信も通知には出ますが、表示名はやはり「匿名」になります
- 既読状態は**今ログインしている人ごと**に覚えます。デモ用アカウントを切り替えると、それぞれ別々に未読が付きます

---

## 8. 自分たちの仕組みの副作用に、対処しようとして見送った

ラベルで絞り込む仕組みは便利ですが、**同時に視野を狭めます。** 課題④で挙げた「同じ情報ばかり見てしまう」現象を、私たち自身が作り出すことになります。この問題自体は、今も残っている課題として認識しています。

### 一度は「2割まぜる」機能を作りました

対処として、**選んだラベルに合わない投稿を、結果の2割まぜて差し込む**機能を実装しました。タイムラインに流れる10件のうち2件は、自分が選んでいないラベルの投稿になる、という仕組みです。一般的なSNSが利用者を長く留まらせるために好みに合うものを見せ続けるのに対し、これはその逆で滞在時間を下げる方向に働きます。**収益を目的としない大学のコミュニティだからこそ実装できる機能**だと考えていました。

### バグが見つかり、削除しました

「提供します」「求めています」をタブで分けたあと（§5）、この機能に不具合が見つかりました。混ぜる投稿を選ぶとき、投稿の方向（提供します/求めています）を区別せずに20%を選んでいたため、選ばれた投稿がたまたま片方の方向に偏ると、もう片方のタブには混ざった投稿が0件になることがありました。「2割まぜる」という約束が、タブ単位では成立しないことがあったわけです。

締切までの残り時間を考え、**直すより先に削除する判断をしました。** 「企業には作れない機能」として目玉に据えていた機能だっただけに、この判断には迷いがありましたが、**壊れた状態のまま目玉機能として発表するより、正直に「見つけて、削った」と説明するほうがよい**と考えました。

- 数字で偏りの度合いを示す案（「あなたは投稿の12%を見ています」）も検討しましたが、分母の取り方が難しく、根拠の弱い数字になるため見送っています
- 視野の偏りという課題自体への対処は、今回の実装には含まれていません。実装するなら、タブごとに独立して20%を計算する、という直し方になります（原因は特定済みです）
...
これはデフォルトの話で、**投稿・返信は匿名にすることも選べます**（§7）。ただし匿名は表示上の見た目だけで、内部的には誰が書いたかを常に持ったままです。聞きづらい質問は名前を隠して聞ける一方、なりすまし対策や状態変更などの権限判定は、匿名にしても普段どおり本人にひもづいたまま動きます。
...
| C | 非公開の返信、匿名投稿 | 初期実装済み。範囲外を混ぜる仕組みは不具合により削除（§8） |
| D | 全体の枠組み、共通部品、サンプルデータの作成、返信の通知 | 完了。以降は合流作業とみんなからの相談を受ける役 |
...
| 期間内に終わらない | 目玉機能でも、不具合が見つかったら直すより先に削って発表に間に合わせる（§8で実際に判断した） |
...
4. **目玉機能でも、壊れていたら削った** ── 視野の偏り対策（§8）は「企業には作れない機能」として目玉に据えていましたが、タブ分けとの組み合わせで不具合が見つかり、直すより先に削除しました。見つかった不具合の中身と、直さず削るという判断をした理由をそのまま説明します
5. **自分たちの仕組みの副作用に、自分たちで向き合った** ── ラベルで絞る便利さと視野が狭まる危険（§8）、身元が分かる安心感と匿名で聞きたい話（§7）。どちらも便利さと引き換えの副作用で、完全には解決していないものも含めて正直に扱っています
...
erDiagram
    ...
    POST {
        ...
        string body
        int price "モノのときだけ使う"
        string place "受け渡し場所。情報のときだけ任意"
        string status "募集中 / やり取り中 / 完了"
        datetime created_at
        datetime updated_at "最終編集日時"
        boolean is_anonymous "trueなら投稿者本人以外には名前を隠す"
    }
    POST_TAG {
        string post_id
        string label_id
    ...
    REPLY {
        ...
        boolean is_anonymous "is_privateとは別軸。表示だけを隠す"
        datetime created_at
    }
    ANNOUNCEMENT {
        string id
...
| POST | 投稿。§3 の2軸（direction と kind）を持つ。`is_anonymous`は表示だけを隠す（§7） |
| POST_TAG | 「どの投稿に」「どのタグが」付いているか |
| REPLY | 投稿への返信。`is_private` で公開／非公開が決まる。`is_anonymous`は別軸（§7） |
